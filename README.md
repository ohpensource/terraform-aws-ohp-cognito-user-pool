...
| [aws_cognito_user_pool_domain.amz_domain](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/cognito_user_pool_domain) | resource |
...
| <a name="input_amz_domain_prefix"></a> [amz\_domain\_prefix](#input\_amz\_domain\_prefix) | A domain prefix for the Amazon Cognito domain - conflicts with Custom DNS | `string` | `null` | no |
...
| <a name="input_client_enable_token_revocation"></a> [client\_enable\_token\_revocation](#input\_client\_enable\_token\_revocation) | app client enable-token-revocation boolean to set up | `bool` | `false` | no |
...
| <a name="input_client_supported_identity_providers"></a> [client\_supported\_identity\_providers](#input\_client\_supported\_identity\_providers) | List of provider names for the identity providers that are supported on this client | `list(string)` | <pre>[<br>  "COGNITO"<br>]</pre> | no |
...
| <a name="input_create_custom_dns_record"></a> [create\_custom\_dns\_record](#input\_create\_custom\_dns\_record) | Create DNS record for custom domain | `bool` | `false` | no |
| <a name="input_create_dummy_record"></a> [create\_dummy\_record](#input\_create\_dummy\_record) | Create dummy DNS record for Cognito authentication | `bool` | `true` | no |
...
| <a name="input_domain"></a> [domain](#input\_domain) | Cognito User Pool domain zone name e.g. ohpen.com | `string` | `null` | no |
...
| <a name="input_domain_hostname"></a> [domain\_hostname](#input\_domain\_hostname) | Cognito user pool hostname e.g. auth | `string` | `"auth"` | no |
...
| <a name="input_password_policy_minimum_length"></a> [password\_policy\_minimum\_length](#input\_password\_policy\_minimum\_length) | The minimum length of the password policy that you have set | `number` | `10` | no |
...
| <a name="input_user_pool_name"></a> [user\_pool\_name](#input\_user\_pool\_name) | (Required) The name of the user pool | `string` | n/a | yes |
...
| <a name="input_username_configuration"></a> [username\_configuration](#input\_username\_configuration) | The Username Configuration. Seting `case_sensitive` specifies whether username case sensitivity will be applied for all users in the user pool through Cognito APIs | `map(any)` | <pre>{<br>  "case_sensitive": true<br>}</pre> | no |
