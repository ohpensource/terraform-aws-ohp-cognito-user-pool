...
variable "username_configuration" {
  description = "The Username Configuration. Seting `case_sensitive` specifies whether username case sensitivity will be applied for all users in the user pool through Cognito APIs"
  type        = map(any)
  default = {
    case_sensitive = true
  }
...
#
variable "domain" {
  description = "Cognito User Pool domain zone name e.g. ohpen.com"
  type        = string
  default     = null
}
...
variable "domain_hostname" {
  description = "Cognito user pool hostname e.g. auth"
  type        = string
  default     = "auth"
}

variable "create_custom_dns_record" {
  description = "Create DNS record for custom domain"
  type        = bool
  default     = false
}
